...
def build_model(hidden_layer_sizes ,input_dim ,X ,Y ,persentage_test ,loss):

    end_idx=len(X)*1-persentage_test
    
    X_train = X[:end_idx]
    Y_train = Y[:end_idx]
    y_train = np_utils.to_categorical(copy.deepcopy(Y_train),num_classes=CANT_CLASES)
...
KERAS_MODEL = build_model(HIDDEN_LAYER_SIZES ,INPUT_DIM ,X ,Y ,conf.TEST_PERCENT,LOSS_FUNCTION)


forxren = FORxREN().extract_rules(KERAS_MODEL ,X ,Y ,INPUT_DIM ,conf.FIRST_LAYER_SIZE ,
                                    conf.EXECUTION_MODE, conf.TEST_PERCENT, conf.MAX_FIDELITY_LOSS, ATTRIBUTES)
